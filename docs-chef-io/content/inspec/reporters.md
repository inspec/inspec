...
Configure the reporter(s) using the `--reporter` option or as part of the general configuration file using the `--config` (or `--json-config`, prior to v3.6) option. Or it could also be configured by using both the options together. While you can configure multiple reporters to write to different files, only one reporter can output to the screen(stdout).
