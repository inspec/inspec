...
{{% alert title="Important Notice" color="info" %}}
Starting with Chef InSpec 6.9.xx, license key enforcement has been removed.
Users no longer need to provide a license key to run Chef InSpec.
However, acceptance of the Chef End User License Agreement (EULA) is still required.
The documentation below applies to Chef InSpec 6.6.0 through 6.8.24.
{{% /alert %}}

Before running Chef InSpec, you must accept the Chef EULA.  
**Note:** Starting with Chef InSpec 6.9.xx, a license key is no longer required.
