...
class ParallelCli < Minitest::Test
  ...
  def test_parallel_dry_run_with_verbose_option
    ...
    stdout = out.stdout
    assert_includes stdout, "complete-profile -t ssh://vagrant@127.0.0.1:2201 --reporter cli --no-create-lockfile --no-sudo --winrm-transport negotiate --insecure false --winrm-shell-type powershell --auto-install-gems false --distinct-exit true --diff true --sort-results-by file --filter-empty-profiles false --reporter-include-source false"
    assert_includes stdout, "control-tags -t ssh://vagrant@127.0.0.1:2201 --reporter cli --winrm-transport negotiate --insecure false --winrm-shell-type powershell --auto-install-gems false --distinct-exit true --diff true --sort-results-by file --filter-empty-profiles false --reporter-include-source false"
    assert_equal stdout.split("\n").count, 7
    assert_exit_code 0, out
  end
end
