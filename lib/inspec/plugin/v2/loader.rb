  ...
  class Loader
    ...
    def load_all
      ...
      registry.plugin_names.dup.each do |plugin_name|
        ...
        begin
          ...
          plugin_details.load_exception = e
          Inspec::Log.error "Could not load plugin #{plugin_name}: #{e.message}"
        end
    ...
    def activate_managed_gems_for_plugin(plugin_gem_name, version_constraint = '> 0')
      ...
      list_managed_gems.each do |spec|
        if Gem::Specification.load spec.gem_dir
          ...
          Dir.chdir(spec.gem_dir) do |_dir|
            managed_gem_set.add_vendor_gem(spec.name, spec.gem_dir)
          end
        end
      end
      ...
      begin
        ...
        ex = Inspec::Plugin::V2::LoadError.new(e.message)
        raise ex
      end
    ...
    def annotate_status_after_loading(plugin_name)
      ...
      when :bundle
        ...
        unless status.installation_type == :bundle
          raise "I only know how to annotate :bundle plugins when trying to load plugin #{plugin_name}"
        end
      end
    end
      ...
      conf_file.each do |plugin_entry|
        ...
        when :user_gem
          ...
          unless plugin_entry[:version].nil? # safe check very rare case.
            ...
            status.description = fetch_gemspec(File.join(plugin_gem_path, 'gems', plugin_name_with_version, '/',
                                                         status.name.to_s + '.gemspec'))&.summary
          end
        when :path
