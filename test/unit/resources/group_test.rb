...
describe "Inspec::Resources::Group" do
  ...
  it "verify group on ubuntu" do
    resource = MockLoader.new(:ubuntu1804).load_resource("group", "root")
    _(resource.exists?).must_equal true
    _(resource.gid).must_equal 0
  end

  it "verify group on ubuntu with mixed case" do
    resource = MockLoader.new(:ubuntu1804).load_resource("group", "GroupWithCaps")
    _(resource.exists?).must_equal true
    _(resource.gid).must_equal 999
  end

  it "verify group on ubuntu with members" do
    resource = MockLoader.new(:ubuntu1804).load_resource("group", "www-data")
    _(resource.exists?).must_equal true
  ...
  it "verify group on ubuntu" do
    resource = MockLoader.new(:ubuntu1804).load_resource("group", "nogroup")
