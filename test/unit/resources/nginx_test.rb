...
describe 'Inspec::Resources::Nginx' do
  describe 'NGINX Methods' do
    ...
    it 'Verify nginx parsing `pid_path` - `/var/run/nginx.pid`' do
      resource = load_resource('nginx')
      _(resource.pid_path).must_match '/var/run/nginx.pid'
    end
  ...
  describe 'NGINX Paramaters' do
    ...
    it 'Verify nginx parsing `user` - `nginx`' do
      resource = load_resource('nginx')
      _(resource.user).must_match 'nginx'
    end

    it 'Verify nginx parsing `group` - `nginx`' do
      resource = load_resource('nginx')
      _(resource.group).must_match 'nginx'
    end
