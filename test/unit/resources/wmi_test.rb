...
describe "Inspec::Resources::WMI" do
  ...
  it "fail wmi on ubuntu" do
    resource = MockLoader.new(:ubuntu1804).load_resource("wmi", { class: "win32_service", filter: "name like '%winrm%'" })
    _(resource.resource_failed?).must_equal true
    _(resource.resource_exception_message)
      .must_equal "Resource `wmi` is not supported on platform ubuntu/18.04."
  end
end
