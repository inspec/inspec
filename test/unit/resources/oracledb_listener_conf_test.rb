...
describe "Inspec::Resources::OracledbListenerConf" do
  it "verify listener settings of oracle DB in linux" do
    resource = MockLoader.new(:centos7).load_resource("oracledb_listener_conf", "$ORACLE_HOME/network/admin/listener.ora")
  ...
  it "verify listener settings of oracle DB in windows" do
    resource = MockLoader.new(:windows).load_resource("oracledb_listener_conf", "$ORACLE_HOME\\network\\admin\\listener.ora")
    _(resource.params["DEFAULT_SERVICE_LISTENER"]).must_equal "XE"
    _(resource.params["EM_EXPRESS_PORT"]).must_equal "5500"
  end
end
