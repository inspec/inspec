...
control "aws_security_group properties" do
  ...
  describe aws_security_group(fixtures['ec2_security_group_alpha_group_id']) do
    ...
    its('inbound_rules.count') { should cmp 2 } # 2 explicit, one implicit
...
control "aws_security_group matchers" do
  describe aws_security_group(fixtures['ec2_security_group_alpha_group_id']) do
    ...
    it { should_not allow_in(port: 631) }
    ...
    # This may be misleading - it DOES allow out 6003
    # as part of a range of ports.
    it { should_not allow_out(port: 6003)}
